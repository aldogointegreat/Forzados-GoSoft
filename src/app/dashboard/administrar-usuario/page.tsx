...
const Page = () => {
	interface User {
		...
		apeMaterno: string;
		areaId: string;
		grupoId: string; // Cambiar a string
	...
	const filteredUsers = useMemo(() => userData.filter((row) => row.nombre.toLowerCase().includes(userSearch.toLowerCase())), [userSearch, userData]);
			...
			<div className="space-y-4">
				...
				<Table
					columns={[
						...
						{ key: "dni", label: "RUT / DNI / Pasaporte", className: "min-w-[200px]" },
						{ key: "correo", label: "Correo" },
						{ key: "puestoDescripcion", label: "Puesto" },
						{ key: "areaDescripcion", label: "Area" },
						{ key: "estado", label: "Estado" },
						{ key: "gruponombre", label: "Grupo", className: "min-w-[150px]" },
					]}
