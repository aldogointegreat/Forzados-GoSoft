...
<html lang="es">
        ...
        <title>La Solicitud de Forzado ah sido Rechazada</title>
        <style>
          body {
          ...
          .container {
            width: 90%;
             max-width: 700px;  /* -- Puedes usarlo si quieres limitar ancho máximo  */
            margin: 20px auto;
...
<body>
        <div class="container">
          ...
          >
            ...
            <tr>
              <td style="text-align: left;"><b>Motivo de rechazo:</b></td>
              <td style="text-align: right;">${solicitud.motivoRechazoDescripcion}</td>
            </tr>
            <tr>
...
export async function POST(request: Request) {
	...
	try {
		...
		const result = await pool.request().input("id", id).input("motivoRechazo", motivoRechazo).input("usuario", usuario).query(`
                ...
                SET ESTADOSOLICITUD = 'RECHAZADO-FORZADO',
                    MOTIVORECHAZO_A_ID = @motivoRechazo,
                    FECHA_RECHAZO_A = GETDATE(),
                    USUARIO_RECHAZO_A = @usuario,
                    ACTION_TOKEN = ''
		...
		const mailOptionsAprobador: MailOptions = {
			...
			subject: "[APP FORZADOS] Solicitud de Forzado Rechazada",
