...
interface Solicitud {
	...
	solicitudId?: number; // Ajustado a camelCase
	...
	observadoEjecucion?: string | null; // Already allows null, but make optional
	desObservacionEjecucion?: string | null; // Already allows null, but make optional
...
export async function POST(request: Request) {
	...
	try {
		...
		const result = await pool.request().input("id", id).input("observacionEjecucion", observacionEjecucion).input("usuario", usuario).query(`
                ...
                SET OBSERVADO_B = 1,
                    OBSERVACION_EJECUCION_B = @observacionEjecucion,
                    FECHA_OBSERVACION_B= GETDATE(),
                    USUARIO_OBSERVACION_B = @usuario,
		...
		const mailOptionsAprobador: MailOptions = {
			...
			subject: "[APP FORZADOS] Solicitud de retiro observada durante Ejecución",
        ...
        <div class="container">
          ...
          >
            ...
            <tr>
                ...
                <td style="text-align: right;">${solicitud.observacionesB}</td>
            </tr>
            <tr>
               <td style="text-align: left;"><b>Descripción de la ejecución:</b></td>
               <td style="text-align: right;">${solicitud.desObservacionEjecucion}</td>
             </tr>
            <tr>
