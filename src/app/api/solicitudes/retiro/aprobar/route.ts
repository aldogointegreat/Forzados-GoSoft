...
export async function POST(request: Request) {
	...
	try {
		...
		const result = await transaction.request().input("id", id).input("usuario", usuario).query(`
                ...
                SET ESTADOSOLICITUD = 'APROBADO-RETIRO', 
                    FECHA_APROBACION_B = GETDATE(),
                    ACTION_TOKEN = ''
    ...
    WHERE SF.SOLICITANTE_B_ID IS NOT NULL 
    AND SF.SOLICITUD_ID = @id
            `);
		...
		if (solicitudResult.recordset.length > 0) {
			...
			const mailOptions: MailOptions = {
				...
				subject: "[APP FORZADOS] Solicitud de Retiro Aprobada",
		...
		return NextResponse.json({ success: true, message: "Registrado Correctamente" });
	} catch (error) {
		await transaction.rollback();
