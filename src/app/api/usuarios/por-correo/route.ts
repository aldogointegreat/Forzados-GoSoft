...
export async function POST(req: NextRequest) {
  try {
    ...
    const userData = {
      ...
      email: recordset[0].CORREO,
      area: recordset[0].ADESC,
      grupo: recordset[0].GRUPDESC,
      grupoid: recordset[0].GRUPO_ID,
      estado: recordset[0].ESTADO, // Add ESTADO field
      roles: recordset[0].ROLES_JSON ? JSON.parse(`{${recordset[0].ROLES_JSON}}`) : {},
      flagNuevoIngreso: recordset[0].FLAG_INGRESO,
      jwt: "1", // Adjust as needed
    };

    // Optionally, validate ESTADO here
