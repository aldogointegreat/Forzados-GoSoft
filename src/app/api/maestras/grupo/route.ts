...
export async function GET() {
	try {
		...
		const { recordset } = await pool.request().query("SELECT * FROM MAE_GRUPO WHERE ESTADO = 1");

		const grupos = recordset.map((singleValue) => {
			return {
				id: singleValue.GRUPO_ID,
				descripcion: singleValue.NOMBRE,
			};
		});
		return NextResponse.json({ success: true, values: grupos });
	} catch (error) {
...
export async function POST(request: Request) {
	try {
		...
		const result = await pool
			...
			.query("INSERT INTO MAE_GRUPO (NOMBRE, USUARIO_CREACION, USUARIO_MODIFICACION, FECHA_CREACION, FECHA_MODIFICACION, ESTADO) VALUES (@descripcion, @usuario, @usuario, GETDATE(), GETDATE(), 1)");
...
export async function DELETE(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("id", id)
			.input("usuario", usuario)
			.query("UPDATE MAE_GRUPO SET ESTADO = 0, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE GRUPO_ID = @id");
...
export async function PUT(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("descripcion", descripcion)
			.input("usuario", usuario)
			.query("UPDATE MAE_GRUPO SET NOMBRE = @descripcion, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE GRUPO_ID = @id");
