...
export async function POST(request: Request) {
	try {
		...
		const result = await pool
			...
			.query("INSERT INTO MAE_AREA (DESCRIPCION, USUARIO_CREACION, USUARIO_MODIFICACION, FECHA_CREACION, FECHA_MODIFICACION, ESTADO) VALUES (@descripcion, @usuario, @usuario, GETDATE(), GETDATE(), 1)");
...
export async function DELETE(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("id", id)
			.input("usuario", usuario)
			.query("UPDATE MAE_AREA SET ESTADO = 0, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE AREA_ID = @id");
		if (result.rowsAffected[0] > 0) {
...
export async function PUT(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("descripcion", descripcion)
			.input("usuario", usuario)
			.query("UPDATE MAE_AREA  SET DESCRIPCION = @descripcion, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE AREA_ID = @id");
		if (result.rowsAffected[0] > 0) {
			return NextResponse.json({ success: true, message: "Record updated successfully" });
		} else {
