...
export async function GET() {
	try {
		...
		const { recordset } = await pool.request().query("SELECT * FROM IMPACTO WHERE ESTADO = 1");

		const impactos = recordset.map((singleValue) => {
			return {
				id: singleValue.IMPACTO_ID,
				descripcion: singleValue.DESCRIPCION,
			};
		});
		return NextResponse.json({ success: true, values: impactos });
	} catch (error) {
...
export async function POST(request: Request) {
	try {
		...
		const result = await pool
			...
			.query("INSERT INTO IMPACTO (DESCRIPCION, USUARIO_CREACION, USUARIO_MODIFICACION, FECHA_CREACION, FECHA_MODIFICACION, ESTADO) VALUES (@descripcion, @usuario, @usuario, GETDATE(), GETDATE(), 1)");
...
export async function DELETE(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("id", id)
			.input("usuario", usuario)
			.query("UPDATE IMPACTO SET ESTADO = 0, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE IMPACTO_ID = @id");
...
export async function PUT(request: Request) {
	try {
		...
		const result = await pool
			...
			.input("descripcion", descripcion)
			.input("usuario", usuario)
			.query("UPDATE IMPACTO SET DESCRIPCION = @descripcion, USUARIO_MODIFICACION = @usuario, FECHA_MODIFICACION = GETDATE() WHERE IMPACTO_ID = @id");
