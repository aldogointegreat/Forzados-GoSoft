...
const ModalCreacionUsuario: React.FC<ModalCreacionUsuarioProps> = ({ isOpen, onClose, isEditing, userData, onSubmit }) => {
  ...
  const [grupos, setGrupos] = useState<{ id: number; descripcion: string }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const fetchGrupos = async () => {
        ...
        const data = await response.json();
        console.log("Grupos fetched:", data.values);
        setGrupos(data.values || []);
      };

 

      await Promise.all([fetchAreas(), fetchPuestos(), fetchGrupos()]);
      ...
      if (isEditing && userData) {
        ...
        setFormData({
          ...
          id: userData.id,
          grupoId: userData.grupoId ? userData.grupoId.toString() : "",
        });
      } else {
        resetForm();
      }
            ...
            <form onSubmit={handleSubmit}>
              {isEditing && (
                <button
                  ...
                  className="mb-4 px-4 py-2 bg-yellow-500 text-white rounded-md text-lg focus:outline-none focus:ring-0"
                >
              ...
              <input
                ...
                className="w-full px-3 py-2 border rounded focus:outline-none focus:ring-2 border-gray-300 mb-4"
              />
              <label className="block text-sm font-medium text-gray-700">RUT / DNI / Pasaporte *</label>
              <input
                type="text"
                name="dni"
                value={formData.dni}
              ...
              <div className="flex justify-between mt-4">
                <button
                  type="button"
                  onClick={handleClose}
                  className="px-4 py-2 bg-gray-300 text-gray-800 hover:bg-gray-400 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500"
                >
                  Cerrar
                </button>
                <button
                  ...
                  className={`px-4 py-2 rounded-md focus:outline-none focus:ring-2 ${
                    isFormValid && !isSubmitting ? "bg-blue-500" : "bg-gray-500"
                  } text-white`}
