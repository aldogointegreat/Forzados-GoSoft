...
const Navbar: FC = () => {
  ...
  const handleLogout = async () => {
    ...
    try {
      // Llamar a doLogout del hook para limpiar el estado local
      doLogout();
      console.log("Estado local limpiado con doLogout");

      // Cerrar la sesión con next-auth, sin redirección automática inicialmente para depuración
      const result = await signOut({ redirect: false });
      console.log("Resultado de signOut:", result);

      // Forzar la redirección manualmente después de cerrar sesión
      window.location.href = "/auth/ingresar";
      console.log("Redirigiendo manualmente a /auth/ingresar");
    } catch (error) {
      console.error("Error al cerrar sesión:", error);
      // Mostrar un mensaje de error al usuario si falla
      alert("No se pudo cerrar la sesión. Por favor, intenta de nuevo o recarga la página.");
    }
  };
  ...
  return (
    <nav className="sticky top-0 z-30 flex h-14 w-full items-center justify-between border-b border-gray-200 bg-gray-50 px-4 shadow-sm">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-base font-medium text-gray-700">Forzados MGFSN</h1>
      </div>
