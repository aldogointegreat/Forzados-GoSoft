...
const Login = () => {
	...
	const { user, fetchUserFromServer } = useUserSession();
	const [showPassword, setShowPassword] = useState(false);
	const [loading, setLoading] = useState(false);

	// Log user session whenever it changes
	useEffect(() => {
		if (user) {
	...
	const handleLogin = async (e: React.FormEvent) => {
	 ...
	 const newErrors = { username: "", password: "" };
		...
		setTimeout(() => {
			setLoading(false);
			if (user && user.role === 1) {
				router.push("/dashboard/solicitud-forzado");
			} else {
			...
			<div className="relative z-10 flex w-full items-center justify-center p-6">
				<div className="w-full max-w-md space-y-6 bg-white p-8 rounded-lg shadow-lg">
					...
					<form className="space-y-6" onSubmit={handleLogin}>
						...
						<button  disabled={loading}  type="submit" className="w-full flex items-center justify-center px-4 py-4 text-white bg-sky-950 hover:bg-sky-800 rounded-lg transition duration-200 focus:outline-none">
