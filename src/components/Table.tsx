...
const Table: React.FC<TableProps> = ({
    ...
    const [localSortCol, setLocalSortCol] = useState<string>(sortColumn);
    const [localSortDir, setLocalSortDir] = useState<SortDir>(sortDirection);
    ...
    const activeSortCol = onSort ? sortColumn : localSortCol;
    const activeSortDir = onSort ? sortDirection : localSortDir;

    const effectiveRows = onSort
      ? rows
      : [...rows].sort((a, b) => compareValues(a[localSortCol], b[localSortCol], localSortDir));
					...
					<tr className="border-b-2 border-gray-200 text-sm text-left">
						{columns.map((column) => (
							<th
								...
								className={`p-3 font-semibold text-gray-600 relative cursor-pointer ${column.className || ""}`}
							>
								<div className="flex items-center justify-between">
									<span>{column.label}
										{activeSortCol === column.key && (
											<span className="ml-1">
												{activeSortDir === "asc" ? "▲" : "▼"}
											</span>
										)}
					...
					{effectiveRows.length > 0 ? (
						effectiveRows.map((row) => (
							<tr key={String(row.id)} className="border-b border-gray-200 hover:bg-gray-100">
								{columns.map((column) => (
									<td key={column.key} className={`p-3 text-sm ${column.className || ""}`}>
