...
const ModalCreacionPuesto: React.FC<ModalCreacionPuestoProps> = ({
  isOpen,
  onClose,
  isEditing,
  puestoData,
  ...
  const [turnos, setTurnos] = useState<{ id: number; descripcion: string }[]>(
    []
  );
  ...
  useEffect(() => {
    ...
    const fetchTurnos = async () => {
      try {
        const response = await fetch("/api/maestras/turno");
        if (!response.ok) throw new Error("Failed to fetch turnos");
        const data = await response.json();
        setTurnos(data.values || []);
      } catch (error) {
  ...
  useEffect(() => {
    if (isEditing && puestoData) {
      const initialData = {
        ...
        turnos: puestoData.turnos || [],
      };
      setFormData(initialData);
      setInitialFormData(initialData);
      setAprobadorNivel(
        puestoData.aprobadorNivel ? puestoData.aprobadorNivel.split(",") : []
      );
    } else {
      resetForm();
  ...
  const handleTurnoChange = (turnoId: number) => {
    const turnoSeleccionado = turnos.find((t) => t.id === turnoId);
    if (!turnoSeleccionado) return;

    setFormData((prevFormData) => {
      const exists = prevFormData.turnos.some((t) => t.id === turnoId);
      const updatedTurnos = exists
        ? prevFormData.turnos.filter((t) => t.id !== turnoId)
        : [...prevFormData.turnos, turnoSeleccionado];

      return { ...prevFormData, turnos: updatedTurnos };
    });
  };
  ...
  const addRole = (
    updatedRoles: { [key: string]: { id: number; descripcion: string } },
    newRole: { id: number; descripcion: string }
  ) => {
    const nextIndex = Object.keys(updatedRoles).length;
    updatedRoles[nextIndex] = newRole;
    return updatedRoles;
  };
  ...
  const removeRole = (
    ...
    const updatedRolesArray = Object.entries(updatedRoles).filter(
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ([_, role]: [string, { id: number; descripcion: string }]) =>
        role.id !== roleId
    );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const dataToSend = {
      ...
      turnos: formData.turnos.map((turno) => turno.id), // Send only the IDs
    };

    console.log("Payload enviado al API:", dataToSend);
    ...
    try {
      ...
      if (!response.ok) throw new Error("Failed to save puesto");
      const result = await response.json();
      if (response.ok) {
        onSuccess(); // Let the parent handle the popover
      } else {
        throw new Error(result.message || "Error en la operación");
      }
    } catch (error) {
      console.error("Error submitting form:", error);
      // Let the parent handle the error popover
      onSuccess(); // Still call onSuccess to refresh data, parent will show error
    } finally {
      resetForm();
      onClose();
    }
        ...
        <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          <div className="p-8 relative">
            ...
            <h2 className="text-2xl font-bold mb-4">
              {isEditing ? "Modificación" : "Creación"}
            </h2>
            <form onSubmit={handleSubmit}>
                  ...
                  <div className="flex flex-col">
                    <label className="inline-flex items-center">
                      <input
                        ...
                        name="aprobadorNivel"
                        value="BAJO"
                        checked={aprobadorNivel.includes("BAJO")}
                        onChange={handleAprobadorNivelChange}
                    ...
                    <label className="inline-flex items-center">
                      <input
                        ...
                        name="aprobadorNivel"
                        value="MODERADO"
                        checked={aprobadorNivel.includes("MODERADO")}
                        onChange={handleAprobadorNivelChange}
                        className="form-checkbox"
                      />
                    ...
                    <label className="inline-flex items-center">
                      <input
                        ...
                        name="aprobadorNivel"
                        value="ALTO"
                        checked={aprobadorNivel.includes("ALTO")}
                        onChange={handleAprobadorNivelChange}
                        className="form-checkbox"
                      />
              ...
              <div className="mb-4 grid grid-cols-2 gap-4">
                {turnos.map((turno) => (
                  <div key={turno.id} className="flex items-center mb-2">
                    <input
                      type="checkbox"
                      id={`turno-${turno.id}`}
                      checked={formData.turnos.some((t) => t.id === turno.id)}
              ...
              <div className="flex justify-between mt-4">
                <button
                  type="button"
                  onClick={handleClose}
                  className="px-4 py-2 bg-gray-300 text-gray-800 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-500 focus:bg-gray-400"
                >
                  Cancelar
                </button>
                <button
