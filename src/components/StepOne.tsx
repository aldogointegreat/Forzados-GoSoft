...
const StepOne: React.FC<StepOneProps> = ({
	...
	useEffect(() => {
			...
			if (id) {
				try {
					...
					if (result.success && result.data.length > 0) {
						...
						setCircuito(String(solicitud.circuito || ""));
					} else {
		...
		fetchSolicitudData();
	}, [setTagPrefijo, setTagCentro, setTagSubfijo, setDescripcion, setDisciplina, setCircuito]);

	console.log("Valor de tagSubfijo en StepOne:", tagSubfijo); // Depuración

	return (
		<form className="space-y-6">
			<div>
				<label className="block text-lg font-medium text-gray-600 mb-2">
					<b> Fecha y Hora de la Solicitud:</b> {currentTime} <br></br>
			...
			<div>
				...
				<select
					...
					required
				>
					<option disabled value="">Seleccione Sub Área</option>
			...
			<div>
				...
				<select
					...
					required
				>
					<option disabled value="">Seleccione Disciplina</option>
			...
			<div>
				...
				<select value={circuito} onChange={(e) => setCircuito(e.target.value)} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
					<option disabled value="">Seleccione Circuito</option>
